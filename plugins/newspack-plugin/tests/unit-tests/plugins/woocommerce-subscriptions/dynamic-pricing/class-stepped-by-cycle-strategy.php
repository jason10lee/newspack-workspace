...
class Newspack_Test_Stepped_By_Cycle_Strategy extends WP_UnitTestCase {
	public function test_applies_to_any_context_with_completed_cycles_signal() {
		...
		$this->assertTrue( $s->applies_to( $this->ctx( 'some_future_surface_trigger', [ 'completed_cycles' => 2 ] ), [] ), 'The cycle signal is the dependency, not a surface allowlist.' );
	...
	public function test_decide_abstains_on_base_equal_amount_when_price_does_not_persist() {
		$s = new Stepped_By_Cycle_Strategy();
		$params = [ 'steps' => [ [ 'at' => 1, 'calc_type' => Amount_Calculator::FIXED_PRICE, 'value' => 1, 'label' => 'Intro' ] ] ];
		// Stateless surface (persists_price = false): abstaining IS the catalog price.
		$this->assertNull( $s->decide( $this->ctx( Subscription_Surface::TRIGGER_SCHEDULED_STEP, [ 'completed_cycles' => 1 ], 1.0 ), $params ) );
	}

	public function test_decide_emits_base_equal_restore_decision_when_price_persists() {
		$s = new Stepped_By_Cycle_Strategy();
		$params = [ 'steps' => [
			[ 'at' => 1,  'calc_type' => Amount_Calculator::FIXED_PRICE, 'value' => 5, 'label' => 'Intro' ],
			[ 'at' => 13, 'calc_type' => Amount_Calculator::PERCENT_OF_BASE, 'value' => 100, 'label' => 'Full price' ],
		] ];
		// Price-persisting surface: the cycle-13 restore step resolves to base ($10)
		// and MUST be emitted — abstaining would freeze the $5 written at cycle 1.
		$d = $s->decide(
			$this->ctx( Subscription_Surface::TRIGGER_SCHEDULED_STEP, [ 'completed_cycles' => 13 ], 10.0, true ),
			$params
		);
		$this->assertNotNull( $d, 'Restore-to-base decisions must be emitted on price-persisting surfaces.' );
	...
	private function ctx( string $trigger, array $signals, float $base = 10.0, bool $persists_price = false ): Pricing_Context {
		...
		return new Pricing_Context( $trigger, $product, null, $base, $signals, null, Pricing_Context::INTENT_RENEWAL, $persists_price );
	}
}
