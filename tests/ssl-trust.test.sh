...
( cd "$FIX"
  ...
  || { echo "FAIL: openssl cert setup failed"; exit 1; }

source "$BIN/ssl-trust.sh"

# ssl_cert_is_host_trusted uses `mkcert -CAROOT`; stub mkcert to point at our fixture CA dir.
# export -f is only needed for any bash -c subshell invocations; harmless no-op otherwise.
...
# ALT lives under FIX so the existing trap handles cleanup automatically.
ALT="$FIX/alt"; mkdir -p "$ALT"; cp "$FIX/otherCA.pem" "$ALT/rootCA.pem"
...
# Regression: no-arg call must return 1 (not abort) under set -u (exercises ${1:-} fix).
ssl_cert_is_host_trusted; ok "no-arg call returns 1 under set -u (not abort)" "$?" "1"
