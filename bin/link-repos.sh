...
# Path vars (PLUGINS_PATH/THEMES_PATH/REPOS_PATH) come from
# resolve-project-path.sh. Skip the container-only sources when all three are
# already set (e.g. by tests/link-repos.test.sh), so this script is sourceable
# on the host without those container paths. errexit/nullglob move into main()
# (below) so sourcing doesn't alter a sourcing shell's options.
if [ -z "${PLUGINS_PATH:-}" ] || [ -z "${THEMES_PATH:-}" ] || [ -z "${REPOS_PATH:-}" ]; then
...
main() {
	...
	local WP_PATH="${1:-/var/www/html/wp-content}"
	if [ ! -d "${WP_PATH}" ]; then
